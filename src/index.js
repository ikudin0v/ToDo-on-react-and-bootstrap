    ...
    <div className="container-sm">
        ...
        <div>
            ...
            <ul className="list-group">
                {taskList.filter(item => item.state === "current").map((task) => (
                    <li className="list-group-item bg-warning bg-opacity-50 border border-warning" key={task.taskId}>
                        <div className="input-group p-1">
                            <button type="button" className="btn btn-success" onClick={() => {doneTask(task.taskId)}}><SvgDoneComponent /></button>
                            <input type="text" id={"input"+task.taskId} className="form-control" placeholder={task.text} disabled/>
                            <button type="button" className="btn btn-warning" onClick={() => {editTask(task.taskId)}}><SvgEditComponent /></button>
                            <button type="button" className="btn btn-danger" onClick={() => {removeTask(task.taskId)}}><SvgRemoveComponent /></button>
                        </div>
                    </li>
                ))}
            </ul>
        ...
        <ul className="list-group">
            {taskList.filter(item => item.state === "done").map((task) => (
                <li className="list-group-item bg-success bg-opacity-50 border border-success" key={task.taskId}>
                    <div className="input-group p-1">
